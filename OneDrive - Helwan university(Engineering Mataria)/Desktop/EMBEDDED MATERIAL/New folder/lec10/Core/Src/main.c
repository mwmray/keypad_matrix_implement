...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
Keypad_Matrix_t key = { .Row_Port = GPIOB, .Row_Start_Pin = 12,
		.Column_Port = GPIOB, .Column_Start_Pin = 6, .Rows = 4,
		.Columns = 4 };
/* USER CODE END PD */
...
int main(void) {
	...
	while (1) {
		...
		if (Keypad_Matrix_ReadKey(&key, 4) && Keypad_Matrix_ReadKey(&key, 5)) {
